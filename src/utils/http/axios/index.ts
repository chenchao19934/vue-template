...
import { joinTimestamp, formatRequestDate, getHeaders } from './helper';
...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res: AxiosResponse, options: RequestOptions) => {
    ...
    // 这里逻辑可以根据项目进行修改
    const hasSuccess = data && Reflect.has(data, 'result_code') && code == ResultEnum.SUCCESS;
    if (hasSuccess) {
      return result;
    }
  ...
  requestInterceptors: (config, options) => {
    ...
    //时间戳
    const { stamp, nonce, sign } = getHeaders();
